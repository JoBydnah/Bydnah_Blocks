...
public class FracturedPlainsBiome extends BydnahBlocksModElements.ModElement {
	public static Biome biome;

	public FracturedPlainsBiome(BydnahBlocksModElements instance) {
		super(instance, 659);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(12638463).setWaterColor(-16724839).setWaterFogColor(329011)
						.withSkyColor(7972607).withFoliageColor(-13382656).withGrassColor(9470285).build();
				...
				mobSpawnInfo.withSpawner(EntityClassification.CREATURE, new MobSpawnInfo.Spawners(EntityType.CHICKEN, 5, 4, 8));
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.NONE).depth(0.1f).scale(0.2f).temperature(0.5f)
						.downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("bydnah_blocks:fractured_plains"));
			}
		}
	}
